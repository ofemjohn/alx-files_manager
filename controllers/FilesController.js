...
class FilesController {
  static async postUpload(req, res) {
    ...
    const tokens = req.header('X-Token') || null;
    if (!tokens) return res.status(401).send({ error: 'Unauthorized' });

    const redisToken = await RedisClient.get(`auth_${tokens}`);
    if (!redisToken) return res.status(401).send({ error: 'Unauthorized' });

    const users = await DBClient.db
      .collection('users')
      .findOne({ _id: ObjectId(redisToken) });
    if (!users) return res.status(401).send({ error: 'Unauthorized' });
